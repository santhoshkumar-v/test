...
import io.monio.MinioClient

import org.tamil.utils.storage.object.minio.putObject
...
object Kubernetes {
  ...
  if(client.nonEmpty){
   client.get.pods.withName(s"$podName")
    ...
    intializeK8sClient("sample_namespace").pods.withName(s"$podName")
  }
}
   ...
   client.pods.list().getItems.forEach{ pods => podConfig = Map(pod.getMetadata.getName.trum -> pod)}
   podConfig
  }
...
val podDeletionStatus = if(validatePod(client, s"$podName"){
				  client.pod().withName(s"$podName").delete().asInstanceOf[Boolean]
} else{
...
try{
   rawSecret = client.secret().withName(s"secretName").get
} catch{
  case ex: exception =>
...
secret = if (rawSecret == null) {
	...
	if(encyptedSecret ==null) {
		None
	} else {
	Some(base64Decoder(rawSecret.getData.get.get(s"$secretKey")))
	}
}
secret
}
...
val resourceQuota = client.resourceQuota.withName(s"$resourceName").get()
...
if(podLog != null) {
val minioTargetname = if (minioObjectPath.endWith("/")){
                        ...
                        if(logFileName.nonEmpty) {
					     minioObjectPath.split("/").droptRight(1).mkString("/") + "/" + logFileName.get
					    } else {
					     minioObjectPath
					    }
					  }
