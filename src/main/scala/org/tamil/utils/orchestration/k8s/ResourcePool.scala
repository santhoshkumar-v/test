...
import io.fabric.kubernetes.api.model.Quantity.getAmountInBytes
...
def fetchResourceReady(spark: SparkSession,
                       namespace: String,
					   resourceName: String,
					   requestedResource: ResourceQuota) :Boolean = {
...
val reqExecutorRequestCpu = spark.conf.get("spark.kubernetes.executor.limit.cores")
